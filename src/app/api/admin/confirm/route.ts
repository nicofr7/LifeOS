...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!email || (plan !== "pro" && plan !== "lifetime")) {
      return NextResponse.json({ error: "Invalid request. Provide email and plan (pro or lifetime)." }, { status: 400 })
    }
    ...
    // Send confirmation email to customer (forwarded to admin since free Resend plan only allows sending to account owner)
    if (resend) {
      ...
      await resend.emails.send({
        ...
        to: ADMIN_EMAIL,
        subject: `📧 [FORWARD TO ${email}] Welcome to LifeOS ${planName}! Your account is now active`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: #eff6ff; border: 1px solid #3b82f6; border-radius: 8px; padding: 15px; margin: 20px 0;">
              <h4 style="margin: 0 0 10px 0; color: #1d4ed8;">📋 For Admin: Forward to Customer</h4>
              <p style="margin: 5px 0; color: #374151; font-size: 14px;">
                Forward this email to <strong>${email}</strong> to confirm their Pro account is active.
              </p>
            </div>
    ...
    return NextResponse.json({
      success: true,
      message: `User ${email} upgraded to ${plan}. Confirmation email sent.`,
    })
  } catch (error: any) {
